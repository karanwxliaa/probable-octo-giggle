...
def load_all_data_ub( root_dir):
    ...
    for folder_name in os.listdir(images_dir):
        ...
        for i, row in av_annotations.iterrows():
            ...
            #expr_label = expr_annotations[i][0]
...
def load_ConFAR_UB( image_size, aug):
    ...
    return val_datasets, val_datasets, task_output_space

    
    
# def load_ConFAR_AF2(image_size, aug):
#     train_transform, val_transform = load_transforms(image_size=image_size, aug=aug)
...
#         if key == 'FER':
#             FERdata = load_all_data_af2(root_dir=affwild2, task='FER')
#             train_dataset, val_dataset = train_test_split(FERdata, test_size=0.2, random_state=42)
#             train_dataset = UnifiedDataset(data=train_dataset, transform=train_transform, task='FER')
#             val_dataset = UnifiedDataset(data=val_dataset, transform=val_transform, task='FER')
            
#             logging.info("Loaded Affwild2 for FER")
#             print("Loaded Affwild2 paths for FER")
        
#         elif key == 'AV':
#             AVdata = load_all_data_af2(root_dir=affwild2, task='AV')
#             train_dataset, val_dataset = train_test_split(AVdata, test_size=0.2, random_state=42)
#             train_dataset = UnifiedDataset(data=train_dataset, transform=train_transform, task='AV')
#             val_dataset = UnifiedDataset(data=val_dataset, transform=val_transform, task='AV')
            
#             logging.info("Loaded Affwild2 for AV")
#             print("Loaded Affwild2 paths for AV")
        
#         elif key == 'AU':
#             AUdata = load_all_data_af2(root_dir=affwild2, task='AU')
#             train_dataset, val_dataset = train_test_split(AUdata, test_size=0.2, random_state=42)
#             train_dataset = UnifiedDataset(data=train_dataset, transform=train_transform, task='AU')
#             val_dataset = UnifiedDataset(data=val_dataset, transform=val_transform, task='AU')
            
#             logging.info("Loaded Affwild2 for AU")
#             print("Loaded Affwild2 paths for AU")
        
#         train_datasets[key] = AppendName(val_dataset, key)
