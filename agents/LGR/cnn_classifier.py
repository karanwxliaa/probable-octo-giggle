...
class CNNClassifier(ContinualLearner, Replayer):
    ...
    def __init__(self, image_size, classes, latent_space, binaryCE=False, binaryCE_distill=False, AGEM=False, 
        ...
        self.conv1 = nn.Conv2d(image_channels, self.out_channels, self.kernel_size)
    ...
    def train_a_batch(self, x, y, scores=None, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, task=1):
        ...
        if x_ is not None:
            ...
            for replay_id in range(n_replays):
                ...
                if (y_ is not None) and (y_[replay_id] is not None):
                    if self.binaryCE:
                        binary_targets_ = lgr_utils.to_one_hot(y_[replay_id].cpu(), y_hat.size(1)).to(y_[replay_id].device)
                ...
                if (scores_ is not None) and (scores_[replay_id] is not None):
                    ...
                    kd_fn = lgr_utils.loss_fn_kd_binary if self.binaryCE else lgr_utils.loss_fn_kd
        ...
        if x is not None:
            ...
            if self.binaryCE:
                ...
                binary_targets = lgr_utils.to_one_hot(y.cpu(), y_hat.size(1)).to(y.device)
