...
def run(args, verbose=False):
    ...
    if verbose:
        # -main model
        if args.latent_replay == "on": 
            lgr_utils.print_model_info(top_model, title="TOP")
            if args.pretrained_root != "none":
                print("ROOT = " + args.pretrained_root)
                # utils.print_model_info(root_model, title="ROOT= "+str(args.pretrained_root))
            else: 
                lgr_utils.print_model_info(root_model, title="ROOT")
        else: 
            lgr_utils.print_model_info(model, title="MAIN MODEL")
        # -generator
        if generator is not None:
            lgr_utils.print_model_info(generator, title="GENERATOR")
