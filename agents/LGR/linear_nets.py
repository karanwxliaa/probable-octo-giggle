...
class fc_layer(nn.Module):
    ...
    def __init__(self, in_size, out_size, nl=nn.ReLU(),
        ...
        if isinstance(nl, nn.Module):
            ...
            self.nl = nn.ReLU() if nl == "relu" else (nn.LeakyReLU() if nl == "leakyrelu" else lgr_utils.Identity())
...
class MLP(nn.Module):
    ...
    def __init__(self, input_size=1000, output_size=10, layers=2, hid_size=1000, hid_smooth=None, size_per_layer=None,
        ...
        for lay_id in range(1, self.layers+1):
            ...
            if lay_id==self.layers and output in ("logistic", "gaussian"):
                layer = fc_layer_split(
                    ...
                    nl_mean=nn.Sigmoid() if output=="logistic" else lgr_utils.Identity(),
                    nl_logvar=nn.Hardtanh(min_val=-4.5, max_val=0.) if output=="logistic" else lgr_utils.Identity(),
                )
        ...
        if self.layers<1:
            self.noLayers = lgr_utils.Identity()
