...
import tqdm
import copy
from . import lgr_utils
from .data import SubDataset, ExemplarDataset
from . import ContinualLearner
...
def train_cl(model, train_datasets, replay_mode="none", scenario="class",classes_per_task=None,iters=2000,batch_size=32,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        for batch_index in range(1, iters_to_use+1):
            ...
            iters_left -= 1
            if iters_left==0:
                data_loader = iter(lgr_utils.get_data_loader(training_dataset, batch_size, cuda=cuda, drop_last=True))
                ...
                if scenario=="task":
                    ...
                    for task_id in range(up_to_task):
                        ...
                        if iters_left_previous[task_id]==0:
                            data_loader_previous[task_id] = iter(lgr_utils.get_data_loader(
                    ...
                    if iters_left_previous==0:
                        ...
                        data_loader_previous = iter(lgr_utils.get_data_loader(ConcatDataset(previous_datasets),
...
def pretrain_root(root_model, model, pretrain_dataset, batch_iterations=1000, batch_size=32, n_classes=10): 
    ...
    for batch_index in range(1, batch_iterations+1):
        iters_left -= 1
        if iters_left==0:
            data_loader = iter(lgr_utils.get_data_loader(pretrain_dataset, batch_size, cuda=cuda, drop_last=True))
            iters_left = len(data_loader)
