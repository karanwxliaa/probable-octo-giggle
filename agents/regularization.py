...
class L2(NormalNN):
    ...
    def learn_batch(self, train_loader, val_loader, task_no):
        ...
        logging.info('#reg_term: %s', len(self.regularization_terms))

        # 1.Learn the parameters for current task
        super(L2, self).learn_batch(train_loader, val_loader, task_no)
    ...
    def criterion(self, inputs, targets, tasks, regularization=True, **kwargs):
        ...
        if regularization and len(self.regularization_terms) > 0:
            # Calculate the reg_loss only when the regularization_terms exist
            reg_loss = 0
            ...
            #print("REG COFF = ",self.config['reg_coef'],"REG LOSS = ",reg_loss)
            #loss += self.config['reg_coef'] * reg_loss

            loss += self.config['reg_coef'] * reg_loss
            # NOT SURE IF THIS IS RIGHT CUZ THEN WONT IT ONLY TAKE THE FIRST TERM?
        return loss
...
class EWC(L2):
    ...
    def calculate_importance(self, dataloader):
        ...
        if self.n_fisher_sample is not None:
            ...
            logging.info('Sample %s for estimating the F matrix.',self.n_fisher_sample)
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            preds = self.forward(input.float()) #changed this
            #print("PREDS = ",preds)
