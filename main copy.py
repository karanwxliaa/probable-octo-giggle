...
import agents
from dataloaders.base import load_ConFAR,load_ConFAR_AF2,load_ConFAR_UB
import warnings
...
def run(args):
	...
	if args.bound == 'UB':
		...
		if args.dataset == 'single':
			train_dataset_splits, val_dataset_splits, task_output_space = load_ConFAR_AF2(image_size=args.image_size, aug=args.train_aug)
		else:
	...
	for i in range(len(task_names)):
		...
		# print("%s\n%s\n%s",len(train_loader),len(val_loader), i)
