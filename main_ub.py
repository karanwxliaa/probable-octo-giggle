...
import agents
from ConFAR.dataloaders.base_dev import load_ConFAR
import torch
...
def run(args):
	...
	logging.info("------------------------------------------------------------")
	print("------------------------------------------------------------")
	task_names = sorted(list(task_output_space.keys()))
	print('Task order:', task_names)
	logging.info('Task order: %s', task_names)

    # Code for agent configuration remains unchanged
	...
	for i in range(len(task_names)):
		...
		# Accumulate datasets for all tasks up to the current one
		accumulated_train_datasets = []
		for j in range(i + 1):  # Include all previous tasks' data
			train_name = task_names[j]
			accumulated_train_datasets.append(train_dataset_splits[train_name])

		# Merge accumulated datasets
		combined_train_dataset = torch.utils.data.ConcatDataset(accumulated_train_datasets)
		...
		train_loader = torch.utils.data.DataLoader(combined_train_dataset,
												batch_size=args.batch_size, 
												shuffle=True, 
												num_workers=0, 
												collate_fn=custom_collate_fn)

		# # Proceed with training using the combined dataset
		# train_loader = torch.utils.data.DataLoader(combined_train_dataset,
		# 											batch_size=args.batch_size, shuffle=True, num_workers=0)

		# Validation data loading remains unchanged
		val_loader = torch.utils.data.DataLoader(val_dataset_splits[task_names[i]],
													batch_size=args.batch_size, shuffle=False, num_workers=0)

		# Learning and evaluation code remains unchanged
...
def custom_collate_fn(batch):
    """
    Custom collate function to pad target tensors to the same size.
    """
    inputs, targets, task_names = zip(*batch)
    # Pad the targets to have the same size
    targets_padded = pad_sequence(targets, batch_first=True, padding_value=0)
    # Stack inputs as usual
    inputs_stacked = torch.stack(inputs, 0)
    # Convert task_names to a tensor or leave as is, depending on your model's needs
    
    return inputs_stacked, targets_padded, task_names
...
def get_args(argv):
	...
	parser.add_argument('--dataset', type=str, default='RAFDB', help="RAFDB|BP4D| DISFA | AffWild2")
...
if __name__ == '__main__':
	...
	logging.info("got args, entered main")
	...
	for reg_coef in args.reg_coef:
		...
		for r in range(args.repeat):
			...
			for i in range(len(task_names)):
				...
				logging.info('\nTask %s average acc: %s',train_name,avg_acc_history[i])
				print('\nTask', train_name, 'average acc:', avg_acc_history[i])
